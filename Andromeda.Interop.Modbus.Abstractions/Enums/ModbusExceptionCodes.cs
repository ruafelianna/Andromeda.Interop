using Andromeda.Interop.Modbus.Abstractions.Assets;
using Andromeda.SourceGenerators.Enum;

namespace Andromeda.Interop.Modbus.Abstractions.Enums
{
    [HasStrings(
        GenerateFunctions = true,
        StringsClass = nameof(ModbusExceptionCodesStrings),
        StringsNamespace = $"{nameof(Andromeda)}.{nameof(Interop)}.{nameof(Modbus)}.{nameof(Abstractions)}.{nameof(Assets)}"
    )]
